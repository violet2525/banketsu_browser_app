(function(){
    ...
    const notifiDesktop = document.getElementById('notify_desktop');
    ...
    let setNotifyDesktop = function(){
        localStorage.setItem('notifyDesktop', notifiDesktop.checked);
    };
    notifiDesktop.addEventListener('change', setNotifyDesktop, false);
    ...
    window.onload = function(){
        ...
        if(localStorage.notifyDesktop && localStorage.notifyDesktop != 'undefined'){
            notifiDesktop.checked = localStorage.notifiDesktop == 'true' ? true : false;
        }else{
            notifiDesktop.checked = true;
        }
    ...
    let notification = function(elmId, type){
        //アイコン通知
        if(localStorage.notifiIcon && localStorage.notifyIcon == 'true'){
            document.getElementById(type).className='fa fa-exclamation'; 
        }

        //デスクトップ通知
        if(localStorage.notifiDesktop && localStorage.notifiDesktop == 'true'){
            if(aryNotification.indexOf(elmId) != -1){
                //既に通知が出ていた場合
            }else{
                //まだ通知されていない場合
    ...
    web_view.addEventListener("did-get-redirect-details", function(details) {
        //if(details.requestMethod == 'POST'){
